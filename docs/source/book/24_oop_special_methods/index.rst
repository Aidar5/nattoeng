...
Special methods in Python are methods that are responsible for  "standard" possibilities of objects and are called automatically when these possibilities are used. For example, , the expression ``a + b``  where a and b are numbers that is converted to such a call  ``a.__add__(b)``. That is, the special method __add__ is responsible for the addition operation. All special methods start and end with double underlining, therefore in English they are often called dunder methods, shortened from "double underscore".
